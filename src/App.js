  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        {page === "dashboard" && <Dashboard user={user} token={token} refreshAllData={refreshAllData} />}
        {page === "members" && (
          <MemberList
            members={members}
        ...
        {page === "memberDetail" && selectedMember && (
          <MemberDetail member={selectedMember} setPage={setPage} token={token} refreshAllData={refreshAllData} />
        )}
        {page === "mypage" && <MyPage user={user} token={token} refreshAllData={refreshAllData} />}
        {page === "admin" && <AdminPage user={user} token={token} refreshAllData={refreshAllData} />}
        {page === "request" && <RequestPage user={user} token={token} refreshUser={refreshUser} refreshAllData={refreshAllData} />}
        {page === "requestManagement" && <RequestManagementPage user={user} token={token} refreshAllData={refreshAllData} />}
        {page === "productManagement" && <ProductManagementPage user={user} token={token} refreshAllData={refreshAllData} />}
      </Box>
    </Box>
  );
}
